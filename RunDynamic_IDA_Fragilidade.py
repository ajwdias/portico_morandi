...
filtro_terremotos = None  # IDA: usar todos os .AT2 da pasta selecionada
...
# Fatores de escala para a Análise Dinâmica Incremental (IDA)
# Grade piloto: cobre desde a resposta original até níveis severos; ajuste se necessário.
fatores_escala = [1, 2, 4, 6, 8, 12, 16, 24, 32, 40]

# Estados-limite de deriva (%). Valores de trabalho configuráveis.
# IMPORTANTE: confirme a classificação adotada no relatório antes da versão final.
estados_limite_idr_pct = {
    'DS1_Slight': 0.4,
    'DS2_Moderate': 0.6,
    'DS3_Extensive': 1.6,
    'DS4_Complete': 4.0,
...
print(f"\n{'#'*72}")
print("  ANÁLISE DINÂMICA INCREMENTAL (IDA)")
print(f"  Terremotos: {len(terremotos)}")
print(f"  Fatores de escala: {fatores_escala}")
print(f"  Total previsto de análises: {len(terremotos) * len(fatores_escala)}")
...
resultados_tit = [
    "Earthquake", "Analysis", "Scale Factor", "Number of Floors",
...
frame_base = 'InfillFrame' if Infill_wall == 'Yes' else 'BareFrame'
ida_root = f'./Results_Dynamic_Multisismos/{frame_base}/IDA'
os.makedirs(ida_root, exist_ok=True)

# =============================================================================
# LOOP IDA — cada terremoto é reconstruído para cada fator de escala
# =============================================================================

n_total = len(terremotos) * len(fatores_escala)
contador = 0

for idx_sismo, terremoto in enumerate(terremotos):
    nome = os.path.splitext(terremoto)[0]

    for esc in fatores_escala:
        contador += 1
        analise_starttime = datetime.now()
        analise = 'FALHOU'
        failure_reason = ''

        # Cada escala recebe pasta própria, evitando sobrescrita dos recorders.
        frame_folder = f'{frame_base}/IDA/SF_{esc:g}'

        print(f"\n{'#'*72}")
        print(f"  IDA {contador}/{n_total} | Sismo {idx_sismo + 1}/{len(terremotos)}")
        print(f"  {terremoto} | SF = {esc:g}")
        print(f"{'#'*72}")

        # Valores ficam NaN se a preparação falhar antes de serem calculados.
        T1_static = np.nan; T1_post = np.nan
        Sa_T1 = Sa_T_02s = Sa_T_03s = Sa_T_06s = Sa_T_1s = np.nan
        Sv_T1 = Sd_T1 = PGA = PGV = PGD = np.nan
        num_pav = max(1, len(Z_pavs) - 1)

        try:
            # Varredura de parâmetros numéricos para a mesma combinação registro x escala
            for dt_fatorreducao in parametros['dt_fatorreducao']:
                if analise == 'SUCESSO': break
                for tolerancia in parametros['tolerancia']:
                    if analise == 'SUCESSO': break
                    for max_iter in parametros['maxiter']:
                        if analise == 'SUCESSO': break

                        # Reconstrói o modelo do zero: essencial para não acumular dano.
                        ops.wipe()
                        ...
                        analyze_modal(numero_de_modos=numero_de_modos,
            ...
            if analise == 'SUCESSO':
                ...
                MaxDESLOC = modelo2D_deslocamento(terremoto=terremoto, Z_pavs=Z_pavs)
                ...
                resultado_dic = modelo2D_resultados(
                    ...
                    T1_static=T1_static, T1_sismo=T1_post,
                ...
                failure_reason = 'Não convergiu após as estratégias numéricas previstas.'
                # Falha NÃO é resposta zero. Mantém IM conhecida e EDP como NaN.
                resultado_dic = {k: np.nan for k in resultados_tit}
                resultado_dic.update({
                    'Earthquake': terremoto,
                    'Analysis': 'FALHOU',
                    'Scale Factor': esc,
                    'Number of Floors': num_pav,
                    'T1 Initial (s)': T1_static,
                    'T1 Post-Earthquake (s)': T1_post,
                    'Fc (MPa)': fc_normal,
                    'Fy (MPa)': fy_normal_Column_TopBars,
                    'Es (GPa)': Es_normal,
                    'Damping Ratio': eps_amortecimento,
                    'Sa(T1) (g)': Sa_T1,
                    'PGA (g)': PGA,
                    'Infill Wall': Infill_wall,
                })
            ...
            print(f'  *** ERRO: {failure_reason}')

        # Campos específicos para IDA/fragilidade
        resultado_dic['Failure Reason'] = failure_reason
        idr_pct = resultado_dic.get('Max. IDR (%)', np.nan)
        for ds, limite_pct in estados_limite_idr_pct.items():
            resultado_dic[f'{ds} Limit IDR (%)'] = limite_pct
            resultado_dic[f'{ds} Exceeded'] = (
                int(float(idr_pct) >= limite_pct)
                if pd.notna(idr_pct) and analise == 'SUCESSO' else np.nan
            )

        ResultadosFinais.append(resultado_dic)
        remover_analises()

        # Checkpoint a cada análise
        parcial = pd.DataFrame(ResultadosFinais)
        parcial.to_csv(f'{ida_root}/IDA_RESULTS_PARTIAL.csv', index=False)
        print(f'  Checkpoint: {len(ResultadosFinais)}/{n_total}')
...
ResultadosFinais_df.to_csv(f'{ida_root}/IDA_RESULTS_ALL.csv', index=False)
pickle.dump(ResultadosFinais, open(f'{ida_root}/IDA_RESULTS_ALL.pkl', 'wb'))

# Base enxuta pronta para o ajuste das curvas de fragilidade
cols_frag = [
    'Earthquake', 'Analysis', 'Scale Factor', 'Sa(T1) (g)', 'PGA (g)',
    'Max. IDR (%)', 'T1 Initial (s)', 'T1 Post-Earthquake (s)'
]
for ds in estados_limite_idr_pct:
    cols_frag += [f'{ds} Limit IDR (%)', f'{ds} Exceeded']
cols_frag += ['Failure Reason']
ResultadosFinais_df[cols_frag].to_csv(f'{ida_root}/FRAGILITY_DATABASE.csv', index=False)

elapsed = time.time() - start_time
n_ok   = int((ResultadosFinais_df['Analysis'] == 'SUCESSO').sum())
n_fail = int((ResultadosFinais_df['Analysis'] == 'FALHOU').sum())
n_erro = int((ResultadosFinais_df['Analysis'] == 'ERRO').sum())

print(f"\n{'#'*72}")
print("  IDA CONCLUÍDA")
print(f"  Total de análises : {len(ResultadosFinais_df)}")
print(f"  SUCESSO           : {n_ok}")
print(f"  FALHOU            : {n_fail}")
print(f"  ERRO              : {n_erro}")
print(f"  Tempo total       : {elapsed:.1f} s ({elapsed/60:.1f} min)")
print(f"  Banco de fragilidade: {ida_root}/FRAGILITY_DATABASE.csv")
print(f"{'#'*72}\n")
